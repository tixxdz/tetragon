...
BPF_KPROBE(event_wake_up_new_task, struct task_struct *new_task)
{
	...
	probe_read(&pid, sizeof(pid), _(&new_task->tgid));
	...
	if (config && config->tg_cgrp_level > 0) {
		...
		__set_task_cgrpid_tracker(config, new_task, curr);
	...
	parent = __event_find_parent(new_task);
	if (parent) {
		curr->key.pid = pid;
